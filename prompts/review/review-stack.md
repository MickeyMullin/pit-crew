...
- **Do not post automatically when the merge recommendation is "fix before merge".** Write the file as always, print the report in chat as always, then say the report was not posted and ask whether to post it to the top PR. That verdict means the work is expected to change before anyone merges it, so posting it unprompted publishes a list of problems against a stack that is still moving, and pins it to a top PR whose SHAs the fixes will invalidate. Wait for an explicit yes, then post with the command above.
- The other two verdicts are unchanged and post automatically: "approve" is a useful durable record, and "do not merge" is precisely the thing other people need to see before they act on the stack. Only "fix before merge" waits.
