...
- **If the file is absent, fall back to what a prior run posted on the top PR before concluding there is no prior run.** Treat that as a partial history only: under "Output file" a report is posted automatically only when the verdict is "approve" *and* the top PR is not yours, and otherwise only if the user said yes when asked. An absent post therefore means nothing on its own — a prior run may have found plenty and simply not been asked to post it. The file, which is written every run regardless of verdict, is the complete record. It is normally present regardless of which clone or worktree this runs from — output always lives at the fixed path `{{HOME}}/agents/output/`, not anywhere repo-relative — so an absent file means a genuinely different environment: another machine or a headless/cloud run.
...
- **The form does not decide whether to post.** Which form to use and whether posting needs the user's say-so are separate questions, answered by this bullet and the next three respectively. A request-changes review is still gated by the ask below — it is the most consequential of the three, since it blocks the top PR until a human dismisses it, and the whole reason the findings verdicts ask first is that a stack in motion should not be publicly blocked before its author has read the review.
- **Do not post automatically when the merge recommendation is "fix before merge" or "do not merge".** Write the file as always, print the report in chat as always, then say the report was not posted and ask whether to post it to the top PR. Say which form it would take — a request-changes review or a comment — so the answer is given knowing what lands. Wait for an explicit yes, then post in that form.
...
- **"approve" posts automatically only on a stack that is not yours**, and it posts as a real approval review rather than a comment. It is a durable record that the stack was reviewed and found sound, it does not go stale the way a findings list does, and there is nothing in it for the author to respond to first — but none of that is worth anything when the author is you, because it then tells you what you are already reading while adding noise to a thread other people have to scan. GitHub would reject a self-approval in any case.
- **Determine authorship of the top layer's PR before posting anything automatically.** Compare `gh api user --jq '.login'` against `gh pr view <top-pr-number> --json author --jq '.author.login'`. The top PR is the only one this review ever posts on, so it is the only authorship that decides this. Never infer it from the checked-out branch: this prompt checks out the top layer's head branch as a matter of course, so having it checked out says nothing about who wrote it.
- If the top PR is yours and the verdict is "approve", **do not post — not as an approval, not as a comment.** Write the file as always, and say in chat that the stack was reviewed, found sound, and not commented on, naming the stack number, the tip SHA, and where the file was written.
...
- **None of this adds a fix mode.** The authorship check here decides one thing only: whether an approve is posted. It never licenses fixing, committing, or pushing on a stack, and an approval this review submits is not a licence to merge the stack it approved. That prohibition is absolute and is stated above.
...
- How the report was posted: as an approval, as a request-changes review, as a plain comment, or not at all (say why — your own top PR, or a findings verdict awaiting your yes). An approval carrying P3 findings is expected; say so plainly rather than hedging the recommendation above.
