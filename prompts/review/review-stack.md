...
- **Take the stack identity from the invocation.** Accept any of these, and say which form you took it as before acting on it:
  - a stack number, or any PR number in the stack (`482`, `#482`)
  - a branch name belonging to any layer of the stack (`atomic-feedback-lineage`)
  - a GitHub PR URL for any layer (`https://github.com/<owner>/<repo>/pull/482`)
- Resolve a branch name to its PR first — `gh pr view <branch> --json number` — then carry on with that number. If the branch has no PR, it cannot be part of a stack: say so and stop. Resolve a URL by parsing `<owner>/<repo>` and the number from it, and stop if that repo is not the one this working directory points at (`gh repo view --json nameWithOwner`) — this review reads real files at real paths, so it can only review a repo you have a clone of.
- Resolve the whole stack with `gh api "repos/{owner}/{repo}/stacks?pull_request={number}"`, which returns the ordered list of PRs bottom to top plus the stack's trunk target in `base.ref`. A bare number is a PR number first: if that lookup finds no stack, try it as a stack number with `gh api repos/{owner}/{repo}/stacks --jq '.[] | select(.number == <number>)'` before concluding there is nothing to review. The two share one sequence, so a number alone cannot tell you which it is.
...
- The stack is linear: the top layer's head branch already contains every layer below it. **The top layer's head branch is what this review reads** — not the bottom, and not the trunk. That branch is the cumulative tree, and it is the only checkout this review needs.
- **Read it in a throwaway worktree, never by checking it out here.** A stack review should not disturb whatever you are working on, and a checkout in this working copy would do exactly that — or refuse, when the tree is dirty.

```
git worktree prune
git fetch origin
git worktree add --detach {{HOME}}/agents/worktrees/<repo>-<top-branch>-<timestamp> origin/<top-branch>
```

- Detached, and from `origin/<top-branch>` rather than a local branch of the same name: a local copy is exactly what goes stale when a rebase cascades up from a lower layer, and it may be checked out in another worktree, which would make the add fail outright. `git worktree prune` first clears metadata left by any worktree whose directory was removed by hand.
- **Creating and removing that worktree is the only filesystem write this review performs outside its report**, and it is permitted despite the "do not modify files" rule below, which governs the code under review rather than the checkout it is read from. It remains true that this prompt never fixes anything.
- Run the entire review inside the worktree: every `git` command, every file read, every citation check.
- **Cite repo-relative paths, never worktree-absolute ones.** The worktree path is scaffolding and means nothing to a reader of the report or to GitHub.
- The report file still goes to `{{HOME}}/agents/output/`, which is a fixed absolute path and unaffected by where the review ran.
- **Remove the worktree once the report has been written, and posted or declined:** `git worktree remove {{HOME}}/agents/worktrees/<repo>-<top-branch>-<timestamp>`. If it refuses because the worktree holds changes, do **not** pass `--force`: this review writes nothing there, so a refusal means something unexpected is on disk. Say so, and give the path.
- Before reviewing anything, record the short SHA of the worktree's tip (`git rev-parse --short HEAD`, run inside it). Note it up front and use it as the `<ref>` in every citation URL. This is also what makes a later re-review cheap: the next run can diff its own HEAD against the SHA recorded in the prior report.
...
- Do not modify files, commit, or push. The only permitted GitHub write is posting the finished report **once**, on the top layer's PR, in exactly one of the three forms described under "Output file" — no inline review comments, no second post restating the verdict, no edits to any PR body, no merges. The verdict is carried by the form of the post, never by a follow-up message.
...
- **Check comment-block length — it is exempt from the "no builds or tests" rule above**, because it is a read of the diff rather than an execution of anything. Flag any comment block the stack adds whose text runs past 500 characters, counting a run of consecutive single-line comments or one block comment as a single block.
- Scope it to the cumulative diff from the merge base resolved above, not to the repo's default branch unless that is genuinely the stack's trunk. Confirm a flagged block is genuinely added by the stack before reporting it, and attribute it to the layer that introduced it.
...
- **If the file is absent, fall back to what a prior run posted on the top PR before concluding there is no prior run.** Treat that as a partial history only: under "Output file" nothing is posted without the user's explicit yes, whatever the verdict. An absent post therefore means nothing on its own — a prior run may have found plenty, or have been perfectly clean, and simply not been told to post. The file, which is written every run regardless of verdict, is the complete record. It is normally present regardless of which clone or worktree this runs from — output always lives at the fixed path `{{HOME}}/agents/output/`, not anywhere repo-relative — so an absent file means a genuinely different environment: another machine or a headless/cloud run.
- **Look in both places a report can land, not just one.** Under "Output file" the report is posted as an approval, a request-changes review, or a plain comment, depending on what the run found — and the first two are *reviews*, which do not appear in the issue-comments list at all. Checking only comments would miss every approved and every changes-requested run, and report a cold start that is not one.
- Resolve the current reviewer's login with `gh api user --jq '.login'`, then fetch both:
  - `gh api repos/{owner}/{repo}/issues/<top-pr>/comments --paginate`
  - `gh api repos/{owner}/{repo}/pulls/<top-pr>/reviews --paginate`
- Take the **most recent** entry across both lists that is authored by that login and whose body begins with the `Reviewed stack:` marker line. Compare them by timestamp, not by which list they came from. Other people and bots comment on and review these PRs — an unfiltered "latest" is the wrong thing to read, and a report from a different reviewer carries triage decisions and finding ids that this run did not make and cannot verify.
- If nothing in either list matches both filters, there is no usable prior run: do a full cold review and say so. Do not fall back to an entry that matches only one of the two filters.
- Read the prior `Review state` block from whichever source you found, and treat its `tip_sha` and `base_sha` as the inputs to the scoping rules below. If both a local file and a posted report exist and their `tip_sha` values disagree, prefer the **file**: the write happens before the post, so a newer file means the previous run wrote its report but failed to post it. Say so in the closing notes — that prior report never reached the PR, and the reader may want it posted.
...
- Once the file write has completed — and only then, never before it and never concurrently with it — post the same report **once, on the top layer's PR**, passing the file that was just written. The top PR is the one place a reader of the whole stack will look. Do not post it to every layer.
- **The verdict is expressed by the form of the post, not by its text.** Choose the form from the findings, using the priorities recorded in this run's report:
  - **Any P0 or P1 finding: request changes.** `gh pr review <top-pr-number> --request-changes --body-file {{HOME}}/agents/output/STACK-<number>.md`
  - **No findings at all, or P3 findings only: approve.** `gh pr review <top-pr-number> --approve --body-file {{HOME}}/agents/output/STACK-<number>.md`
  - **At least one P2, but nothing above it: plain comment.** `gh pr comment <top-pr-number> --body-file {{HOME}}/agents/output/STACK-<number>.md`
- Never post the report and then say the changes are requested, and never post a review and then repeat the verdict in a separate comment. One post, carrying the whole report as its body.
- **An approval still carries the whole report, P3s included.** A P3 is a lower-risk quality or maintainability note, and whether it is worth fixing now or tracking separately is the author's call — so approve and let them decide. Never approve to be agreeable, and never rank a finding P3 because P3 is the rank that approves: the priority comes from the finding's risk, judged before you know which form it selects. A defect that can produce incorrect behavior for anyone is a P2 at minimum.
- **The form does not decide whether to post.** Which form to use, and whether to post at all, are separate questions: this bullet answers the first, and the next three answer the second.
- **This review never posts on its own initiative, for any verdict.** Write the file as always, print the report in chat as always, then say the report was not posted, name the form it would take — an approval, a request-changes review, or a comment — and ask whether to post it to the top PR. Wait for an explicit yes, then post in that form. A general instruction to review the stack is not consent to post on it; ask each run.
- Each of the three forms has its own reason for asking, and none of them is a formality. A findings verdict means the stack is expected to change before it merges, so posting unprompted publishes a list of problems against work that is still moving and pins it to a top PR whose SHAs the fixes will invalidate. A request-changes review goes further and blocks the top PR until a human dismisses it. And an approval is not the harmless end of the scale it looks like: submitted on someone else's stack it is a real review that can satisfy a required-review gate, so it may be the thing that lets the stack merge. Deciding that on a reviewer's behalf is not this prompt's call to make.
- Naming the form when you ask is the point of asking. "Post the report?" and "submit an approval that may unblock this stack?" are different questions, and only the second one lets the answer mean anything.
- **Determine authorship of the top layer's PR before offering to post anything.** Compare `gh api user --jq '.login'` against `gh pr view <top-pr-number> --json author --jq '.author.login'`. The top PR is the only one this review ever posts on, so it is the only authorship that decides this. Never infer it from the branch under review: this prompt puts the top layer's head branch in a worktree as a matter of course, so having it on disk says nothing about who wrote it.
- If the top PR is yours and the verdict is "approve", **do not offer to post at all** — not as an approval, not as a comment. Write the file as always, and say in chat that the stack was reviewed, found sound, and not commented on, naming the stack number, the tip SHA, and where the file was written. GitHub rejects a self-approval outright, and a self-addressed "looks good" comment only adds noise to a thread other people have to scan. Asking would be offering something that cannot work.
- Taken together: **every post this review makes is one the user asked for**, on any stack and for any verdict. Authorship decides whether the offer is worth making at all, not whether posting happens without one. Either way the file is written every run.
- One case deserves a word rather than a silent decision: a stack whose top PR is yours may still contain layers written by other people, and an approve is the one verdict they might want on the record. When the stack has layers by other authors, say so when you report the clean result and offer to post it — then post only if the user says yes. The approval would still be rejected on your own PR, so offer the comment form there instead, and say why it is not an approval.
- **None of this adds a fix mode.** The authorship check here decides one thing only: whether posting is worth offering. It never licenses fixing, committing, or pushing on a stack, and an approval this review submits is not a licence to merge the stack it approved. That prohibition is absolute and is stated above.
...
11. Comment blocks this stack adds that exceed 500 characters (P3)
...
- Every file:line reference must come from a fresh, direct read of the real file at its real path (Read tool, or `grep -n` / `sed -n` against the actual path on disk) taken immediately before you write the citation down. Never reuse line numbers you saw earlier from a concatenated diff, a `git diff` / `git show` patch, a Read of a scratch/tmp file, or from memory of an earlier tool call in the conversation — those numbering schemes do not match the source file. When the review is running in a worktree, that path is inside the worktree — read it there, and strip the worktree prefix when you write the citation down, which must be repo-relative.
...
- How the report was posted: as an approval, as a request-changes review, as a plain comment, or not at all (say why — your own top PR, or awaiting your yes, which is the normal case). An approval carrying P3 findings is expected; say so plainly rather than hedging the recommendation above.
